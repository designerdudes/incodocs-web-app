...
export default function EditForwarderForm({ params }: Props) {
  ...
  useEffect(() => {
    const fetchForwarderData = async () => {
      try {
        ...
        if (!forwarder) {
          throw new Error("Forwarder data not found in response");
        }

        const formData = {
          forwarderName: forwarder?.forwarderName?.toString() || "",
          ...
          documents: forwarder?.documents?.map((doc: any) => ({
        ...
        setFetchError("Failed to load forwarder details. Please try again.");
  ...
  const saveProgress = (data: any) => {
    try {
      localStorage.setItem("forwarderFormData", JSON.stringify(data));
          ...
          <Button
            onClick={async () => {
              try {
                ...
                await putData(`/shipment/forwarder/put/${forwarderId}`, payload);
                setIsLoading(false);
                GlobalModal.onClose();
                ...
                setIsLoading(false);
                GlobalModal.onClose();
                toast.error(error.message || "Error updating forwarder");
              }
            }}
          >
            Confirm
