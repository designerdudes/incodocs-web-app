...
const formSchema = z.object({
  transporterName: z
    .string()
    .min(3, { message: "Transporter name must be at least 3 characters long" }),
...
export default function EditTransporterForm({ params }: Props) {
  ...
  useEffect(() => {
    const fetchTransporterData = async () => {
      try {
        ...
        if (!transporter) {
          throw new Error("Transporter data not found in response");
        }

        const formData = {
          transporterName: transporter?.transporterName?.toString() || "",
          gstNumber: transporter?.gstNumber || "",
          panNumber: transporter?.panNumber || "",
          tanNumber: transporter?.tanNumber || "",
          addmsme: transporter?.addmsme || "",
          panfile: transporter?.panfile || "",
          tanfile: transporter?.tanfile || "",
          additional: transporter?.additional || "",
          gstfile: transporter?.gstfile || "",
          address: transporter?.address?.toString() || "",
          responsiblePerson: transporter?.responsiblePerson?.toString() || "",
          email: transporter?.email?.toString() || "",
          mobileNo: transporter?.mobileNo?.toString() || "",
          documents: transporter?.documents?.map((doc: any) => ({
        ...
        setFetchError("Failed to load transporter details. Please try again.");
  ...
  const saveProgress = (data: any) => {
    try {
      localStorage.setItem("transporterFormData", JSON.stringify(data));
  ...
  const handleSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    GlobalModal.title = "Confirm Transporter Update";
    GlobalModal.description = "Are you sure you want to update this transporter?";
    GlobalModal.children = (
      <div className="space-y-4">
        <p>Transporter Name: {values.transporterName}</p>
          ...
          <Button
            onClick={async () => {
              try {
                ...
                setIsLoading(false);
                GlobalModal.onClose();
                toast.error(error.message || "Error updating transporter");
              }
            }}
          >
            Confirm
  ...
  if (isFetching) {
    return (
      <div className="flex items-center justify-center h-60">
        ...
        <p className="ml-2 text-gray-500">Loading transporter details...</p>
      </div>
    );
  }
                              ...
                              <div className="flex items-center space-x-2">
                                <FileUploadField
                                  ...
                                  onChange={async (value: string | null) => {
                                    // If value is a file URL, set it in the form
                                    if (typeof value === "string" && value) {
                                      setValue(`documents.${index}.fileUrl`, value, { shouldDirty: true });
                                      saveProgress(getValues());
                                    }
                                  }}
                                  value={form.getValues(`documents.${index}.fileUrl`)}
                                />
                                {field.value && (
                                  <a
                                    href={field.value}
