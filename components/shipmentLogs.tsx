...
export const ShipmentLogs: React.FC<ShipmentLogsProps> = ({
  logs,
  isView,
}) => {
  ...
  const renderValue = (path: string, value: any) => {
    return path.includes("CreatedBy") && value?.username
      ? value.username
    ...
    <Sheet>
      <SheetTrigger asChild>
        <Button
          ...
          className={`${!isView && "m-0 p-2"} text-sm font-normal`}
        >
              ...
              {items.map((item, index) => (
                ...
                >
                  ...
                  <TimelineContent className="text-foreground mt-2 rounded-lg border px-4 py-3">
                    ...
                    <ul className="list-disc text-sm pl-5 mb-3">
                      {logs[index].changes.map((change) => (
                        <li key={change._id}>
                          Updated {change.field.replace(/([A-Z])/g, " $1").trim()}
                        </li>
                      ))}
                    </ul>
                    ...
                    <Collapsible>
                      <CollapsibleTrigger className="flex items-center space-x-2 text-primary hover:underline mt-2">
                        <span>
                          View {logs[index].changes.length} Change
                          {logs[index].changes.length > 1 ? "s" : ""}
                        </span>
