...
import { BrandName, documentationSidebarTabs, factoryManagementSidebarTabs, sidebarTabs } from '@/lib/constants';
...
function DocumentationSidebarTabs() {
    const [hoveredItem, setHoveredItem] = useState(null);
    ...
    const pathname = usePathname()
    const router = useRouter()
    const firstTabs = documentationSidebarTabs.slice(0, 3);
    const restTabs = documentationSidebarTabs.slice(3);
    return (
        <div className='w-64  sticky top-0 h-screen z-50 bg-primary  text-white'>
            <div className='flex items-center justify-start px-4 h-16 bg-primary-dark'>
                <h1 className='text-2xl font-bold'>{BrandName}</h1>
...
export { Sidebar, FactoryManagementSidebarTabs,DocumentationSidebarTabs };
